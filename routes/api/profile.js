...
router.put(
  '/experience',
  auth,
  check('contract', 'Contract title is required').notEmpty(),
...
router.put(
  '/course',
  auth,
  check('institute', 'School is required').notEmpty(),
  check('certificate', 'Degree is required').notEmpty(),
  ...
  async (req, res) => {
    ...
    try {
      ...
      profile.courses.unshift(req.body);
...
/**
 * @route    DELETE api/profile/course/:crc_id
 * @desc     Delete course from profile
 * @access   Private
 */

router.delete('/course/:crc_id', auth, async (req, res) => {
  try {
    const foundProfile = await Profile.findOne({ user: req.user.id });
    foundProfile.courses = foundProfile.courses.filter(
      (crc) => crc._id.toString() !== req.params.crc_id
    );
    await foundProfile.save();
    return res.status(200).json(foundProfile);
  } catch (error) {
